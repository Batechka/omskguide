...
<head>
    ...
    <style>
        .route-hero { background: var(--surface); border-radius: var(--radius); padding: 2rem;  box-shadow: var(--shadow); }
        .route-stats { display: flex; gap: 1.5rem; margin: 1rem 0; font-size: 1.1rem; }
        .stop-card { display: flex; background: var(--surface); border-radius: var(--radius); padding: 1.5rem; margin-bottom: 1.5rem; box-shadow: var(--shadow-sm); }
        .stop-number { font-size: 2.5rem; font-weight: bold; color: var(--primary); min-width: 3.5rem; }
        .stop-content { flex: 1; }
        .stop-meta { color: var(--muted); margin-top: 0.5rem; font-style: italic; }
        .btn-detail { margin-top: 1rem; display: inline-block; }
        .faq-item { margin-bottom: 1.5rem; }
        .faq-question { font-weight: 600; color: var(--primary-dark); margin-bottom: 0.3rem; }
        ...
        .btn-route:hover { background: var(--primary-dark); color: white; }

        @media (max-width: 768px) {
            .stop-layout { grid-template-columns: 1fr; }
            .stop-card { flex-direction: column; }
            .route-hero { padding: 1.5rem; }
        }
    </style>
</head>
<body>
    ...
    <script>
        ...
        init() {
            document.querySelectorAll('.audio-player').forEach(container => {
                ...
                this.players.set(playBtn, { audio, container, playBtn });
